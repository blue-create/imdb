...
sapply(kaggle2, class) # to see current classes
...
docvars(corp_kaggle, "title") <- kaggle1$title ## Add back Title
...
corp_dfm <- dfm(corp_kaggle, 
                ...
                split_hyphens = TRUE,
                remove = stopwords("en"))%>%
dfm_trim(min_termfreq = 1, termfreq_type = "quantile")
  # only keeps top 5% of words - play around with (no doc limit)
#doesn´t work with smaller that 1, as the document is too big
...
mod <- readRDS("data/topic_mod.rds") # add data folder path
#mod <- readRDS("data/topic_mod_demo.rds")
