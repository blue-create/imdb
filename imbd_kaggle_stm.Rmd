...
# test linear model: to test the linear model I would suggest selecting a few countries
...
#USA
#create new binary variable for USA
dat_kaggle$USA<-NA
dat_kaggle$USA[dat_kaggle$country=="USA"]<-1
dat_kaggle$USA=="NA"<-0
dat_kaggle$USA[is.na(dat_kaggle$USA)]<- 0
usa_model <- lm(rating ~ USA + duration + Topic_1 + Topic_2 + Topic_3+ Topic_4+ Topic_5+ Topic_6 + Topic_7 + Topic_8 + Topic_9 + Topic_10, data = dat_kaggle)
summary(usa_model)
stargazer(usa_model, type="text")
