...
# Create df 
titles <- as.data.frame(matrix(titles, ncol = 2, byrow = TRUE), 
...
```{r }
# Call node based on xpath
rating1 <- html_nodes(url1, ".text-muted+ .text-muted , .text-muted+ span , .ratings-bar+ .text-muted , .ratings-imdb-rating strong , .ratings-metascore")
rating2 <- html_nodes(url2, ".text-muted+ .text-muted , .text-muted+ span , .ratings-bar+ .text-muted , .ratings-imdb-rating strong , .ratings-metascore")

# Strip the HTML tags and extract title and year 
r1 <- html_text(rating1)
r2 <- html_text(rating2)
```
