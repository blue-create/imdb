...
table1<-stargazer(topics_only_model,type = "text", out = "Table 1", 
...
unit_fe_kaggle <- lm(rating ~ Topic_1 + Topic_2 + Topic_3 + 
                    ...
                    Topic_9 + Topic_10 + as.factor(country), data = dat_kaggle)
...
stargazer(topics_scraped_model,type = "text", out = "Table 10", 
          title = "Topic Model: Which Topics have better ratingsin 2020?", 
          omit.stat=c("LL","ser","f","adj.rsq"), 
          dep.var.labels.include = FALSE,
          column.labels = c("Rating"))
```
...
=======
          column.labels = c("Rating"))
#Note: USA have lower ratings for 2020 movies, while Canadian movies tend to have a quite high rating. None of the effects are statitically significant.  
```


## Scraped Romance Model
``` {r romance model, warning = FALSE}
# Romance Model
romance_model_2020 <- lm(Rating ~ UK*Topic_7, data = dat_2020)
summary(romance_model_2020)

stargazer(romance_model_2020,
          type = "text", out = "Table 13", 
          title = "How good were British on making romance movies in 2020?",
          omit.stat=c("LL","ser","f","adj.rsq"), 
          dep.var.labels.include = FALSE,
          column.labels = c("Rating in UK"))
# Note: Contraty to what found in the kaggle data, in 2020, romance movies from Britain have a very high negative effect.
```
## Scraped USA Model
```{r USA mode, warning = FALSE}
# USA model
usa_crime_model_2020 <- lm(Rating ~ USA*Topic_9, data = dat_2020)
usa_fiction_model_2020<- lm(Rating ~ USA*Topic_1, data = dat_2020)
summary(usa_crime_model_2020)
summary(usa_fiction_model_2020)
stargazer(usa_crime_model_2020,usa_fiction_model_2020,
          type = "text", out = "Table 14", 
          title = "USA: Crime & Fiction Movies in 2020", 
          omit.stat=c("LL","ser","f","adj.rsq"), 
          dep.var.labels.include = FALSE,
          column.labels = c("Rating of Crime Movies", "Rating of Finction Movies"))
# Note: USA-made killer-cop-gang-serial films make much better than most USA films as happened before. 
```

## Scraped Panel Data Estimation
```{r scraped plm, warning = FALSE}
# PLM
...
stargazer(metascore_model,
          type = "text", out = "Table 16", 
          title = "Overall Effect of Rating with each Topic accounting for country 2020 and Metascore", 
          omit.stat=c("LL","ser","f","adj.rsq"),
          dep.var.labels.include = FALSE,
          column.labels = c("Rating"))
# Note: Including Metascore as a covariate seems to reduce the negative association of the topics except for topic 7, which is now viewed to be worse.
```
>>>>>>> Stashed changes
